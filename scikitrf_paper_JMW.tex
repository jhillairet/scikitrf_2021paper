\documentclass[10pt, letterpaper, twosides]{scrartcl}
...
%
\author{A. Arsenovic, % 810 Labs LLC
	    J. Hillairet, % CEA, IRFM, F-13108 St-Paul-Lez-Durance, France 
		J. Anderson, % Purdue University, West Lafayette, Indiana, USA
		H. Forsten, % TBD
		Vincent Rieß, \\ % TBD
 		Michael Eller, % University of Virginia, Virginia, USA
		Noah Sauber, %University of Virginia, Virginia, USA
		Robert Weikle, %
		William Barnhart %HawkEye 360
	}
...
Scikit-rf uses the \textit{matplotlib} package \cite{Hunter:2007} to generate various different types plots for the attributes of a \texttt{Network}. For example, plotting the ring slot’s scattering parameters on the Smith chart can be done in one line; with the output shown in Fig.~\ref{fig:ringslot_smith}:
